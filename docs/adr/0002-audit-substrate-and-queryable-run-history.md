...
- The index draws from `cost.agents` **and** `preflight.attempts[*].ledger`. Only the final preflight attempt reaches `cost.agents`, so a parse-retry or fallback that ran, resolved to a concrete version and was superseded exists nowhere else — the retry path is exactly where the served identity is most likely to differ from the configured one. The two surfaces overlap by exactly one entry (`preflight.attempts` records the final attempt too, and that attempt *is* the `cost.agents` preflight entry), so the last attempt is skipped when its ledger signature matches a `cost.agents` preflight ledger. The rule is positional and signature-confirmed rather than signature-only: a same-profile parse retry legitimately resolves to the same version as the attempt it replaced, so dropping on a signature match alone would erase a real invocation, while a record whose preflight result never reached `cost.agents` still has its final attempt indexed.
