"""Tests for model profile accumulation — pure aggregation + thin I/O.

Covers what :mod:`theforge.model_profiles_storage` owns after the #2467 split:
folding a run outcome into stored history, the backfill, the reset, and the
merge catalogue — plus the seams where an accumulated profile is then read by a
signal, which is behaviour worth exercising end to end.

Imports still go through the ``theforge.model_profiles`` facade on purpose: that
is the compatibility surface older callers use, and these tests are what keeps
it honest. Signal tests that construct stored state directly live in
``test_model_profiles_read_model.py``.
"""
