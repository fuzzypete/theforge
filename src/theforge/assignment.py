...
def _has_auth(agent: AgentDef, secrets: dict[str, str]) -> bool:
    """Return True if the agent has usable auth.

    Delegates to agent_is_ready(), which merges os.environ with secrets
    (secrets take precedence) and checks the correct invocation path for
    CLI agents.
    """
    profile = agent.to_model_profile()
    ready, _ = agent_is_ready(profile, secrets)
    return ready
...
def _pick_agent(agents: list[AgentDef], tier: str, secrets: dict[str, str]) -> AgentDef | None:
    """Pick cheapest agent of the given tier that has usable auth.

    Skips agents whose auth check fails (API key missing, CLI not found, etc.).
    """
...
def _select_reviewers(
    ...
    # Filter by auth availability — skip agents whose auth check fails
    strong = [a for a in _agents_by_tier(agents, "strong") if _has_auth(a, _secrets)]
    tier_agents = (
        [a for a in _agents_by_tier(agents, tier) if _has_auth(a, _secrets)]
        if tier != "strong"
        else []
    )
    ...
    if exclude_model is not None:
        ...
        if not preferred:
            ...
            preferred = [a for a in agents if _has_auth(a, _secrets) and a.model != exclude_model]
        if not preferred:
...
def assign_models(
    ...
    secrets is merged with os.environ for auth checks (secrets take precedence).
    """
    if not agents:
    ...
    if "dev" in explicit_profiles:
        ...
        # Check promotion
        dev_agent_for_check: AgentDef | None = _pick_agent(agents, dev_base_tier, _secrets)
        ...
        dev_agent = _pick_agent(agents, effective_dev_tier, _secrets)
        if dev_agent is None:
            # Fall back to any authed agent
            authed = [a for a in agents if _has_auth(a, _secrets)]
            if authed:
    ...
    if "preflight" in explicit_profiles:
        ...
        agent = _pick_agent(agents, tier, _secrets)
        if agent is None:
            authed = [a for a in agents if _has_auth(a, _secrets)]
