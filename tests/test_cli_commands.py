...
class TestCmdRunConfigOverrides:
    ...
    def test_reviewers_trims_pool(self, tmp_path):
        ...
        assert passed_config.provenance.resolved_value_sources["review_pool"] == "cli override"
    ...
    def test_max_cycles_override(self, tmp_path):
        ...
        assert (
            passed_config.provenance.resolved_value_sources["retry.max_review_cycles"]
            == "cli override"
        )
...
class TestCmdSprintQueryMode:
    ...
    def test_query_mode_parallel_updates_runtime_config_snapshot(self, tmp_path):
        ...
        assert rc == 0
        context = mock_run_sprint.call_args.args[0]
        assert context.config.sprint.max_parallel == 3
        assert context.config.provenance.resolved_values["sprint.max_parallel"] == 3
        assert (
            context.config.provenance.resolved_value_sources["sprint.max_parallel"]
            == "cli override"
        )
