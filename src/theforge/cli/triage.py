...
def cmd_triage(args: argparse.Namespace) -> int:
    ...
    if args.report:
        try:
            ...
            return 1
        if headless:
            return _cmd_triage_headless(args, config, report=report)
        return _cmd_triage_proposals(args, config, report=report)
    ...
    if headless:
        return _cmd_triage_headless(args, config, report=None)
...
def register_parser(subparsers: object) -> None:
    ...
    p = subparsers.add_parser(
        "triage",
        help=(
            "Generate a backlog report, propose from --report, ratify with --ratify, "
            "or discard a pending decision with --discard "
            "(without a TTY: propose and persist a pending decision)"
        ),
    )
