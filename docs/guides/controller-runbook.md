...
**An unfounded score still opens it.** A degraded preflight, or one that examined
no criteria, still yields a complexity figure — a deliberately *high*,
conservative one (see "Which attempt a resume proceeds from" above). That figure
opens the gate like any other: a story forge could not size is not a story to
spend on unasked, so the conservative number is a reason to put the question to
you rather than to skip it. What changes is what you are told, not whether you
are asked. The provenance appears with the score in `forge status` and is
recorded as `preflight_complexity_gate.score_provenance_note` on the audit —
`degraded preflight` or `preflight examined no criteria` — so you can weigh how
much the number is worth before answering. `opened: false` therefore means one
thing only: the score was under the threshold (or the gate was disabled).
