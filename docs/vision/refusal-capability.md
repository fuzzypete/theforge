...
- **Symptom verification:** when a diagnosed bug ships, review verifies the *symptom* no longer reproduces, not merely that the hypothesized cause was addressed. Refusing to conflate "fixed the guess" with "fixed the problem" is a refusal at the review boundary.
