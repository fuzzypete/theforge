...
- **`assignment_history.yaml` → adaptive router.** The earliest concrete compounding loop in TheForge: per-run model performance is recorded, future assignments adjust. The pattern generalized in v0.13: ADR-0006 defines the router's trust boundary, adaptive assignment now routes on escalation history, model profiles, domain signal, and bounded exploration — and `assignment_history.yaml` itself became a derived view of the audit substrate (#793).
...
- ADRs citing this doctrine now include [ADR-0006](../adr/0006-adaptive-router-trust-boundary.md) (adaptive-router trust boundary) and [ADR-0005](../adr/0005-commit-centric-review-handoff.md) (commit-centric review handoff). Future candidates: autonomy boundary, operator-action work-object type.
