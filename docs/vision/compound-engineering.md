...
- **Layer 1 — Capture.** Stop discarding what we already produce. Story text, plan content, plan regeneration lineage, dev prompts, reviewer reasoning — all of it currently drops on the floor. Layer 1 preserves it in the per-run audit record.
...
- **#1516 diagnosis-staleness check.** A diagnosis written against commit A is mechanically detected as stale when commit B lands. The operator's earlier diagnostic work is preserved (compounding) and explicitly invalidated (refusal-capable) rather than silently rotting. Both axes in one feature.
