...
def cmd_sprint_status(args: object) -> int:
    ...
    if is_live:
        ...
        if entries is not None:
            # Read sprint_name from the state file
            ...
            try:
                ...
                sprint_name = (
                    state_data.get("sprint_name", "") if isinstance(state_data, dict) else ""
                )
            except Exception:
                pass
    else:
    ...
    if entries is None:
        ...
        # Extract sprint name from the summary file
        import yaml  # noqa: PLC0415
        ...
        try:
            ...
            if isinstance(summary_data, dict):
                sprint_name = summary_data.get("sprint", {}).get("name", "")
        except Exception:
            pass
...
def register_sprint_status_parser(subparsers: object) -> None:
