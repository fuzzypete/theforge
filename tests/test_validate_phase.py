...
from theforge.coordinator.gate import _extract_hanging_test, _run_gate_diagnostic_pass
...
# ── Gate-timeout diagnostic re-run pass (issue #1217) ──────────────────────────
...
def _diag_config(tmp_path: Path):
    ...
    return dataclasses.replace(
        ...
        validation=dataclasses.replace(
            config.validation,
            gate_diagnostic_pytest_base="python -m pytest",
...
def test_extract_hanging_test_from_summary_line() -> None:
    ...
    assert _extract_hanging_test(output) == "tests/test_hang.py::test_deadlock"


def test_extract_hanging_test_from_banner_when_no_summary() -> None:
    output = "tests/test_hang.py::test_spin \n+++++++++ Timeout +++++++++\nStack of MainThread"
    assert _extract_hanging_test(output) == "tests/test_hang.py::test_spin"
...
def test_extract_hanging_test_none_when_multiple_timeouts() -> None:
    ...
    assert _extract_hanging_test(output) is None


def test_extract_hanging_test_none_when_clean() -> None:
    assert _extract_hanging_test("42 passed in 3.14s") is None


def test_diagnostic_pass_hang_reproduces_single_threaded(tmp_path: Path) -> None:
    """Hang reproduces under -n 0: the hanging test node is isolated and highlighted."""
    ...
    with patch(
        ...
        telemetry = _run_gate_diagnostic_pass(config, tmp_path, task=task, iter_num=2)
...
def test_diagnostic_pass_hang_does_not_reproduce_single_threaded(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        telemetry = _run_gate_diagnostic_pass(config, tmp_path, task=task, iter_num=1)
...
def test_diagnostic_pass_itself_times_out(tmp_path: Path) -> None:
    ...
    with patch(
        "theforge.coordinator.util._run_shell_detailed",
        return_value=(False, "TIMEOUT after 60s: python -m pytest", None, True),
    ):
        telemetry = _run_gate_diagnostic_pass(config, tmp_path, task=task, iter_num=3)
