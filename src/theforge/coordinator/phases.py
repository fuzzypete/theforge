...
This module implements the REVIEW phase of the coordinator state machine,
plus shared helpers (approve finalization, PR creation, story archival).

The DEV phase lives in phase_dev.py.
The VALIDATE phase lives in phase_validate.py.

Functions that call symbols patched in tests accept a ``mod`` parameter
(the coordinator module) so lookups resolve in the coordinator's namespace
where patches land.
"""
...
# ── Re-exports from phase-specific modules ───────────────────────────
