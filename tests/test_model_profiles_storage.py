"""Accumulation: how a finished run becomes stored profile history (#2467).

Exercises :mod:`theforge.model_profiles_storage` directly — the owner of run
outcomes as *stored state*: folding one run into a profile (``apply_run`` and
its ``_fold_*``/``_update_*``/``_zero_*``/``_recompute_*`` helpers), the YAML
I/O surface, the history backfill, and the ``_merge_*`` catalogue.

Nothing here asks what routing sees. The signals are read from
``model_profiles_read_model`` and tested in
``test_model_profiles_read_model.py`` against directly-constructed state; the
handful of places where an accumulated profile is then read back through the
facade are in ``test_model_profiles.py``. The exception is the harness-
terminated cluster at the end, which keeps its closing
``get_dev_success_rate`` assertion: "an interleaved kill changes nothing the
router sees" is the point of that cluster, and splitting the assertion away
from the runs that set it up would leave neither half meaning anything.
"""
...
from theforge.model_profiles_read_model import get_dev_success_rate
