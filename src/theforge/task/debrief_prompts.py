...
def render_debrief_section(assembled_context: Any, *, style: str = STYLE_YAML_BLOCK) -> str:
    ...
    parts = [
        ...
        *_DISPOSITION_LINES,
        ...
        "with the claim.",
        "",
        "Each pointer must identify ONE specific artifact, not a kind of artifact.",
        "`a commit` and `the test` name something every run has and resolve against",
        "nothing. Prefer an exact path, a section number, or a commit sha. If you",
        "describe the artifact in words instead, EVERY word you use must appear in",
        "the thing you mean — a pointer is matched by containment, not by sharing a",
        "word or two, so `the rebuild entry point` will not match a commit whose",
        "message merely mentions rebuilding. A pointer that cannot be resolved is",
        "recorded as such — that is a fine outcome, and far better than a vague one",
        "that resolves against the wrong artifact.",
        "",
